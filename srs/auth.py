        ...
        if self._path.exists():
            try:
                ...
                if isinstance(data, dict):
                    ...
                    # 重建 (app_id, bus_id) -> user_id 索引
                    self._app_bus_to_uid = {}
                    for name, rec in data.items():
                        app_id = rec.get("app_id", "default")
                        bus_id = rec.get("bus_id")
                        if app_id and bus_id:
...
# 注意：_users 的 key 是 username，但 get_or_create_from_external
    ...
    def get_or_create_from_external(
        ...
        """业务后端三方登录时，根据 (app_id, bus_id) 在本地创建或更新记录。

        - 如果 (app_id, bus_id) 已映射到某 user_id → 直接返回该记录（允许 username 变化）
        - 如果 username 已存在但无 app_id/bus_id 映射 → 补上映射
        - 如果均不存在 → 创建新账号：
            * user_id: user_<uuid>
            * app_id: 业务应用标识
            * bus_id: 业务服务器用户 id（ext_data["id"]）
            * username: 业务后端返回的 username/nickname/id
            * ext_data: 完整业务后端返回的用户信息快照
        """
        bus_id = str(ext_data.get("id") or ext_data.get("userId") or "") or None
        with self._lock:
            # 优先按 (app_id, bus_id) 查——这是跨设备登录同一业务用户时的唯一身份
            if app_id and bus_id:
                existing_uid = self._app_bus_to_uid.get((app_id, bus_id))
                if existing_uid:
                    name = self._user_id_to_name.get(existing_uid)
                    if name:
                        rec = self._users.get(name)
                        if rec:
                            return rec
            ...
            if username in self._users:
                ...
                # 如果有 app_id/bus_id 但记录中没有，补上
                if app_id and bus_id and not rec.get("bus_id"):
                    rec["app_id"] = app_id
                    rec["bus_id"] = bus_id
                    self._app_bus_to_uid[(app_id, bus_id)] = rec["user_id"]
                    # 增量同步 MySQL + JSON
            ...
            self._flush()
            logger.info(
                f"[UserStore] 业务后端用户首次登录，自动创建账号: "
                f"app_id={app_id} bus_id={bus_id} username={username} user_id={user_id}"
            )
            return self._users[username]

    def get_by_app_bus(self, app_id: str, bus_id: str) -> Optional[dict]:
        """按 (app_id, bus_id) 查记录（支撑 bus: 前缀解析）"""
        with self._lock:
            uid = self._app_bus_to_uid.get((app_id, bus_id))
            if uid:
                name = self._user_id_to_name.get(uid)
...
def issue_token(
    ...
    app_id: 业务应用标识，写入 JWT `app` claim，resolve 时作 defaultAppId 来源之一。
    """
    now = int(time.time())
